...
NOTE_CREATE_RE = re.compile(
    r"^\s*(?:(?:создай|добавь|сохрани|запиши)\s+(?:мне\s+)?(?:(?:обычную|новую|личную|отдельную|короткую)\s+)?(?:заметку|запись)\b|"
...
NOTE_CREATE_PREFIX_RE = re.compile(
    r"^\s*(?:(?:создай|добавь|сохрани|запиши)\s+(?:мне\s+)?(?:(?:обычную|новую|личную|отдельную|короткую)\s+)?(?:заметку|запись)|"
    r"(?:(?:создай|создать|сохрани|сохранить|запиши|записать|составь|составить)\s+(?:мне\s+)?(?=список\b))|"
