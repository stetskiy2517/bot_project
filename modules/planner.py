"""AI Smart Planner: intent -> parameters -> clarification -> action."""
...
UPDATE_WORDS = ("перенеси", "перенести", "сдвинь", "сдвинуть", "измени", "изменить", "поменяй", "поменять")
DELETE_WORDS = ("удали", "удалить", "отмени", "отменить", "убери", "убрать")
FREE_WORDS = ("когда свобод", "свободное окно", "свободные окна", "найди время", "найди окно", "куда поставить")
EVENT_WORDS = (
...
def detect_intent(text: str) -> IntentResult:
    """Определить намерение без LLM для безопасных календарных действий."""
    ...
    # Разговорная команда без глагола: «невролог в пятницу в 19».
    # Наличие сущности события + календарного времени достаточно для создания.
...
def _needs_time(text: str) -> bool:
    """Для создания с указанной датой спросить время, если оно отсутствует."""
...
async def handle_text(
    ...
    """Единая точка входа Smart Planner для текста и распознанного голоса."""
    ...
    logger.info("Smart Planner intent=%s confidence=%.2f", intent.name, intent.confidence)
    ...
    if intent.name == INTENT_VIEW:
        await update.message.reply_text("Просмотр календаря подключаю следующим шагом Smart Planner.")
        return True
    if intent.name == INTENT_UPDATE:
        await update.message.reply_text("Перенос и изменение событий подключаю следующим шагом Smart Planner.")
        return True
    if intent.name == INTENT_DELETE:
        await update.message.reply_text("Удаление событий подключаю следующим шагом Smart Planner с подтверждением.")
        return True
    if intent.name == INTENT_FREE:
        await update.message.reply_text("Поиск свободных окон подключаю следующим шагом Smart Planner.")
        return True
