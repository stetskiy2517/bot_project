...
EVENT_CATEGORIES = {
    "work": {"color_id": "3", "keywords": ("работ", "встреч", "созвон", "совещ", "клиент", "офис", "проект", "презентац", "отчет", "отчёт", "коммерчес", "переговор", "планерк", "рассылк", "тендер", "договор")},
    ...
    "personal": {"color_id": "5", "keywords": ("личн", "дом", "покуп", "купить", "куплю", "купи", "магазин", "день рождения", "забрать", "отвезти", "ужин", "обед", "завтрак", "уборк", "стир", "готов", "душ", "свидан", "люблю", "любов")},
}
...
def _resolve_category(
    ...
    *,
    ...
    if user_id is None or category not in {"other", "uncategorized"}:
        return category, color_id

    ai_choice = category_ai.classify_event_category(user_id, text)
    return ai_choice if ai_choice is not None else (category, color_id)
...
def _extract_title(text: str) -> str:
    ...
    title = re.sub(r"^(?:а\s+)?(?:ещ[её]\s+)?", "", title, flags=re.IGNORECASE)
    title = re.sub(
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        event = _build_event(text, start, end, get_category_colors(user_id), user_id=user_id)
