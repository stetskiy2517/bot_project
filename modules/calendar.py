...
from core.user_operations import remember_calendar_effect, confirm_calendar_effect
...
def _create_event(user_id: int, event: dict) -> dict:
    ...
    event = remember_calendar_effect(user_id, event)
    ...
    try:
        ...
        expected = ((event.get("extendedProperties") or {}).get("private") or {}).get("secretaryRequest")
        if not event.get("id") or not expected:
            raise
        try:
            existing = service.events().get(calendarId="primary", eventId=event["id"]).execute()
        except Exception:
            raise
        actual = ((existing.get("extendedProperties") or {}).get("private") or {}).get("secretaryRequest")
        if actual != expected or existing.get("status") == "cancelled":
            raise
        created = existing
    confirm_calendar_effect(user_id, created)
