...
class TelegramAssistantTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_deterministic_reply_is_recorded_for_later_follow_up(self):
        ...
        update = SimpleNamespace(message=message, effective_user=SimpleNamespace(id=42), callback_query=None)
        context = _Context()
    ...
    async def test_no_ai_access_keeps_safe_generic_reply(self):
        ...
        self.assertTrue(handled)
        self.assertEqual(message.sent[-1][0], "Не понял команду. Скажи иначе или уточни, что нужно сделать.")
