...
STATUS = {
    "csrf_token": "synthetic-test-csrf",
    "server_time_ms": 0,
...
def run_case(browser, root, output, case, viewport):
    ...
    context.route("http://assistant.test/", lambda route: route.fulfill(
    ...
    html = html.replace('<script src="/request-client.js"></script>',
                        "<script>" + (root / "web/request_client.js").read_text(encoding="utf-8") + "</script>")
    scripts = "".join(
        "<script>" + (root / "web" / name).read_text(encoding="utf-8") + "</script>"
        for name in ("reminders.js", "library.js", "voice_gesture.js")
    )
