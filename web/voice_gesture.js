(() => {
  function installStableMobileViewport() {
    ...
    window.addEventListener("orientationchange", () => {
      setTimeout(syncStableViewport, 120);
    });
    document.addEventListener("focusin", syncStableViewport);
    document.addEventListener("focusout", () => {
      requestAnimationFrame(syncStableViewport);
    });
    if (window.visualViewport) {
      window.visualViewport.addEventListener("resize", syncStableViewport, {passive: true});
      window.visualViewport.addEventListener("scroll", syncStableViewport, {passive: true});
    }
  }
