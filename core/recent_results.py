"""Short-lived cross-module context for results just shown to the user.

A list shown by calendar/reminders/tasks/notes should support natural follow-ups
such as «второе», «удали второе» or «перенеси второе на 17:00». Only the most
recent list is active, which avoids guessing between different modules.
"""
...
def ordinal_index(text: str) -> int | None:
    """Return a zero-based ordinal found in a short follow-up phrase."""
    match = _ORDINAL_RE.search(_normalise(text))
    if not match:
        return None
...
def remember_recent_results(
    ...
    *,
    ...
    """Remember only stable ids + user-visible labels from the latest shown list."""
    compact = []
