...
DECLARATIVE_FACT_RE = re.compile(
    r"^\s*(?:я|мы)\s+"
    r"(?:(?:(?:обычно|всегда|регулярно|сегодня|завтра|послезавтра|утром|днем|вечером|ночью)|"
    r"кажд\w*\s+(?:день|утр\w*|вечер\w*|ноч\w*)|по\s+(?:будням|выходным)|"
    r"(?:в|к)\s*(?:[01]?\d|2[0-3])(?:(?::|\.)[0-5]\d)?)\s+){0,5}"
    r"(?:принима\w*|пью|пьем|пьём|выпива\w*|ложусь|встаю|медитир\w*|"
    r"читаю|читаем|кормлю|кормим|поливаю|поливаем|выгулива\w*)\b",
    re.IGNORECASE,
)
...
CLEAR_NEW_COMMAND_RE = re.compile(
    ...
    r"когда\b[^.!?]{0,60}\bсвобод\w*\b|есть\s+ли\s+свободн\w*\s+окн\w*|"
    r"(?:найди|найти)\s+(?:встреч\w*|событ\w*|созвон\w*|"
