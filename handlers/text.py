...
def _processing_error(text: str) -> str:
    if detect_input_language(text) == "en":
        return "I couldn't process the message. Please try again."
...
async def handle_message_text(update: Any, context: Any, text: str) -> bool:
    ...
    try:
        ...
        logger.exception("Telegram deterministic routing failed for user %s", user_id)
        await update.message.reply_text(_processing_error(candidate))
        return True
    ...
    await update.message.reply_text(unhandled_reply_for(candidate))
    return True
